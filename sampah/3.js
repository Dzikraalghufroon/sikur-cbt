...
const UjianPage = () => {
  ...
  //   }));
  // };
  // const handleJawabanChange2 = (soalId, jawaban) => {
  //   setSelectedJawaban(prev => ({
  //     ...prev,
  //     [soalId]: jawaban
  //   }));
  ...
  //     [soalId]: jawaban
  //   }));
  //   console.log('Selected Jawaban:', selectedJawaban);
  ...
  const handleJawabanChange = (soalId, jawaban) => {
    ...
    if (!nomorterisi.includes(soalId)) {
      setNomorterisi(prev => {
        const newNomorterisi = [...prev, soalId];
        console.log('Updated Nomorterisi:', newNomorterisi); // Debug log
        return newNomorterisi;
      });
    }
  };
