...
const Mapel = () => {
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.put(`${import.meta.env.VITE_SERVER}/update_kelas`, currentEdit)//('http://localhost:2000/kelas', currentEdit);
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete) {
            try {
                const response = await axios.delete(`${import.meta.env.VITE_SERVER}/delete_kelas/${id}`);//(`http://localhost:2000/kelas/${id}`);
    ...
    return (
        <div>
            ...
            <div className={styles.tambah}>
                <Link to="/kelas/tambah" className={styles.abutton}>Tambah Data</Link>
            </div>
                <h2>Tabel Kelas</h2>
