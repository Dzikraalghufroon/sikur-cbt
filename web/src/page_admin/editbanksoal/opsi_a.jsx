...
const Editor = () => {
    ...
    useEffect(() => {
        const fetchSiswa = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_SERVER}/modul-read-soal/${name}`);
    ...
    const handleUpload = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.put(`${import.meta.env.VITE_SERVER}/modul-update-opsi_a`, { name, opsi_a });
            console.log("Berhasil diupload:", response.data);
            window.confirm('berhasil di update');
        } catch (error) {
        ...
        <div>
            ...
            <div className={styles.editor}>
                <form onSubmit={handleUpload}>
                    <h2>Masukkan opsi_a</h2>
                    {renderEditor(textInputRef, setopsi_a, "Ketik teks di sini...")}
