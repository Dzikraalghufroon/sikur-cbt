...
const UjianPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchdatabasedurasi = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_SERVER}/mulai_ujian/${id}`, {
  ...
  useEffect(() => {
    const fetchDurasi = async () => {
      try {
        ...
        console.log("halo2", response.data)
      } catch (error) {
  ...
  useEffect(() => {
    if (soal.length > 0) {
      const fetchOpsiterisi = async () => {
        ...
        try {
          const opsiterisiResponse = await axios.get(`${import.meta.env.VITE_SERVER}/read_jawaban_pilihan_user/${currentSoalId}/${id}`, { withCredentials: true });
  ...
  const submitJawaban = async () => {
    try {
      ...
      if (selectedJawaban[currentSoalId]) {
        ...
        <div className={Styles.soalSection}>
          {currentSoal && (
            <div>
              ...
              <p className={Styles.waktuUjian}>{formatDurasi(durasi)}</p><br /><br />
